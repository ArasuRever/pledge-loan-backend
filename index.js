...
app.get('/api/customers', async (req, res) => {
  try {
    const allCustomers = await db.query("SELECT id, name, phone_number, address FROM Customers ORDER BY name ASC"); // Exclude image from list
...
app.post('/api/loans', upload.single('itemPhoto'), async (req, res) => {
  ...
  try {
    ...
    const itemImageBuffer = req.file ? req.file.buffer : null; // Get item image buffer
    ...
    // CORRECTED: Insert item_image_data
...
// CORRECTED: Handles item_image_data and converts to Data URL
app.get('/api/loans/:id', async (req, res) => {
  try {
    ...
    if (loanDetails.item_image_data) {
      ...
      let mimeType = 'image/jpeg'; // Basic inference
      if (imageBase64.startsWith('/9j/')) mimeType = 'image/jpeg';
      else if (imageBase64.startsWith('iVBORw0KGgo')) mimeType = 'image/png';
      loanDetails.item_image_data_url = `data:${mimeType};base64,${imageBase64}`;
    }
    delete loanDetails.item_image_data; // Don't send raw buffer
...
// === GET ALL OVERDUE LOANS (FINAL CORRECTED VERSION) ===
